...
import com.ecommerce.theHashed.JWTConfiguration.AuthManager;
import com.ecommerce.theHashed.JWTConfiguration.JwtTokenProvider;
...
public class LoginController {
	...
	public ResponseEntity<?> userLogin(@RequestBody LoginRequest loginRequest) {
	 ...
	 private JSONObject getUserResponse(String token) {
	    	...
	    	try {
	    		CustomerAccount user = userservice.getUserDetailById(_getUserId());
				HashMap<String,String> response = new HashMap<String,String>();
				response.put("user_id", ""+_getUserId());
				response.put("email", user.getEmailId());
				response.put("firstname", user.getFirstName());
				response.put("lastname", user.getLastName());
				response.put("mobile", user.getMobileNo().toString());
